...
*/
            ...
            return true;
        } else if (action.equals("getTopScores")) {
            final String leaderboardId = args.getString(0);
            Log.d(LOG_TAG, String.format("%s", leaderboardId));
            ...
            cordova.getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (getGameHelper().isSignedIn()) {
                        _getTopScores(leaderboardId, delayedCC);
                    } else {
                        //PluginResult pr = new PluginResult(PluginResult.Status.OK);
                        //pr.setKeepCallback(true);
                        //delayedCC.sendPluginResult(pr);
            ...
            return true;
        } else if (action.equals("getLeaderboards")) {
            final CallbackContext delayedCC = callbackContext;
            cordova.getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (getGameHelper().isSignedIn()) {
                        _getLeaderboards(delayedCC);
                    } else {
                        //PluginResult pr = new PluginResult(PluginResult.Status.OK);
                        //pr.setKeepCallback(true);
                        //delayedCC.sendPluginResult(pr);
    ...
    private void _getPlayerScore(String leaderboardId) {
        ...
        Games.Leaderboards.loadCurrentPlayerLeaderboardScore(getGameHelper().getApiClient(), leaderboardId, LeaderboardVariant.TIME_SPAN_ALL_TIME, LeaderboardVariant.COLLECTION_PUBLIC)
                .setResultCallback(new ResultCallbackSubmitScoreResult());
    }

    private void _getLeaderboards(final CallbackContext callbackContext) {
        class ResultCallbackLoadLeaderboards implements ResultCallback<Leaderboards.LeaderboardMetadataResult> {
          @Override
            public void onResult(Leaderboards.LeaderboardMetadataResult result) {
                if (result.getStatus().getStatusCode() == GamesStatusCodes.STATUS_OK) {
                    LeaderboardBuffer leaderboardBuffer = result.getLeaderboards();
                    Iterator<Leaderboard> iterator = leaderboardBuffer.iterator();
                    ...
                    try {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            item.put("name", leaderboard.getDisplayName());
                            item.put("code", leaderboard.getLeaderboardId());
                            results.put(item);
                        }
        ...
        Games.Leaderboards.loadLeaderboardMetadata(getGameHelper().getApiClient(), true).setResultCallback(new ResultCallbackLoadLeaderboards());
    }
                ...
                .setResultCallback(new ResultCallback<Leaderboards.LoadScoresResult>() {
                    ...
                    public void onResult(@NonNull Leaderboards.LoadScoresResult loadScoresResult) {
                        if (loadScoresResult.getStatus().getStatusCode() == GamesStatusCodes.STATUS_OK) {
                            ...
                            Iterator<LeaderboardScore> iterator = scores.iterator();
                            JSONArray results = new JSONArray();
                            try {
                                while (iterator.hasNext()) {
                                    JSONObject playerScore = new JSONObject();
    ...
    private void _getAchievements(final CallbackContext callbackContext) {
        Games.Achievements.load(getGameHelper().getApiClient(), true).setResultCallback(new ResultCallback<Achievements.LoadAchievementsResult>() {
            @Override
            public void onResult(@NonNull Achievements.LoadAchievementsResult loadAchievementsResult) {
                if (loadAchievementsResult.getStatus().getStatusCode() == GamesStatusCodes.STATUS_OK) {
                    ...
                    Iterator<Achievement> achievIterator = loadAchievementsResult.getAchievements().iterator();
                    JSONArray achievements = new JSONArray();
                    try {
                        while (achievIterator.hasNext()) {
                            JSONObject achiev = new JSONObject();
                            Achievement achievement = achievIterator.next();
                            achiev.put("id", achievement.getAchievementId());
                            achiev.put("name", achievement.getName());
                            achiev.put("description", achievement.getDescription());

                            achievements.put(achiev);
                        }

                    } catch (JSONException e) {
                    }

                    PluginResult pr = new PluginResult(PluginResult.Status.OK, achievements);
                    callbackContext.sendPluginResult(pr);
